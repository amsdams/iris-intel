...
- Partially adheres: dynamic request batching exists in core and now drives initial live request waves; `IitcDataCache`
  ...
  before rendering merged responses. Runtime progressive rendering still redraws merged entity snapshots instead of true
  IITC surgical Leaflet layer mutation batches; retry-limit state exists and retry request failures use the same
