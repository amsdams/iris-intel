...
| Map layer registry for overlay/filter configuration | Started | Align with IITC-CE's many-enabled-layer model after the highlighter registry. First pass adds a typed content-side registry for existing layer controls, uses it for UI grouping/labels, and exposes registered layer ids/kinds in diagnostics without changing the layer set. Render ownership, default derivation, and page-runtime filter registration remain hard-coded follow-ups. Existing fields, links, portals, team/level portal filters, ornaments, artifacts, labels, tiles, draw tools, player tracker, missions, selected objects, and user-location layers need fuller registry coverage over time. |
