...
| Map layer registry for overlay/filter configuration | Started / Performance WIP | Align with IITC-CE's many-enabled-layer model. First pass adds a shared typed registry for existing layer controls, uses it for UI grouping/labels/defaults, and exposes registered layer ids/kinds/defaults in diagnostics without changing the layer set. Tile debug, Draw Tools, and player tracker toggles route to their own overlay refresh paths. Core toggles now use scoped sync, secondary-overlay masks, batched removals, and a visibility-only fast path when entity data has not changed. Remaining WIP is moving page-runtime filter ownership toward an IITC-style group/filter-layer model so fields, links, portals, faction filters, and level filters stop relying on generic per-entity sync for common layer-only toggles. |
