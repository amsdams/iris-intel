...
| Portal highlighter registry and selection model | In progress | Align with IITC-CE's one-active-highlighter model before adding new highlighters. First pass registers only existing IRIS highlighter-like behavior: level fill / level color, health fill / needs recharge, and six explicit history selectors for visited, not visited, captured, not captured, scout controlled, and not scout controlled. Key counts stay a layer. Missing resonators, high-level-only, ornament portal coloring, hide team, and other IITC plugins stay deferred until the registry is stable. |
| Map layer registry for overlay/filter configuration | Open | Align with IITC-CE's many-enabled-layer model after the highlighter registry. Existing fields, links, portals, team/level portal filters, ornaments, artifacts, labels, tiles, draw tools, player tracker, missions, selected objects, and user-location layers are still hard-coded across settings, UI labels, diagnostics, and render branches. A registry should describe layer id, group, default visibility, kind, min zoom, and render/filter ownership without changing the current layer set. |
