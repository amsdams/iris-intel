...
| Portal highlighter registry and selection model | In progress | Align with IITC-CE's one-active-highlighter model before adding new highlighters. First pass registers only existing IRIS highlighter-like behavior: level fill / level color, health fill / needs recharge, and six explicit history selectors for visited, not visited, captured, not captured, scout controlled, and not scout controlled. Manual visual check passed after applying the IRIS history color rule: positive states yellow, negative/target states red. Legacy `levelFill`, `healthFill`, and history layer settings are now migration-only and are no longer part of current layer settings. Key counts stay a layer. Missing resonators, high-level-only, ornament portal coloring, hide team, and other IITC plugins stay deferred until the registry is stable. |
| Map layer registry for overlay/filter configuration | Started | Align with IITC-CE's many-enabled-layer model after the highlighter registry. First pass adds a shared typed registry for existing layer controls, uses it for UI grouping/labels/defaults, and exposes registered layer ids/kinds/defaults in diagnostics without changing the layer set. Tile debug, Draw Tools, and player tracker toggles now route to their own overlay refresh paths instead of entity rendering. Render ownership and page-runtime filter registration remain hard-coded follow-ups. Existing fields, links, portals, team/level portal filters, ornaments, artifacts, labels, missions, selected objects, and user-location layers need fuller registry coverage over time. |
