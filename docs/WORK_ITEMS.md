...
| Resume label-heavy overlays after a short settle delay          | Done   | Page-world plugin labels and portal highlights now hide during active movement and restore after a short settle delay                     |
| Suspend plugin marker registries during movement                 | In Progress | Marker-hidden diagnostic showed z8 Normal still at `24ms avg` while player/planned marker registries were hidden or empty; Bench now applies movement mode before the settle window to isolate whether layer-change timing is the remaining cost |
