...
| Review IRIS persisted location/startup map-state ownership               | Open   | after hard refresh IRIS can sometimes reopen around Accra/0-ish fallback instead of the last useful map camera, while Mini-IRIS does not show the same behavior; inspect persisted mapState, Intel startup sync, bounds-less updates, and default/fallback camera precedence                                                        |
