...
| Add compact source publication pass summary         | Done   | page-world source publications now carry a `sourcePass` id/reason/current-or-carry summary in copied preload and timed rows, including moving state, sources touched, real `setData` calls, unchanged skips, and pass `setData` time |
