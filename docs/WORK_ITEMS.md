...
| Rename page-world runtime out of POC module path                         | Done   | manifest-loaded MAIN-world runtime now lives at `src/content/page-map-runtime.ts`; diagnostics still keep explicit POC buttons for isolated probes |
| Add page-world camera sync protocol                                      | In Progress | diagnostics can send the current IRIS camera to the page-world runtime with SYNC CAMERA; page-world moveend posts camera changes back as a diagnostic result |
| Add page-world source sync protocol                                      | In Progress | diagnostics can now send plain-JSON IRIS portal/link/field GeoJSON with SYNC DATA without also forcing a query probe or camera jump |
| Add page-world layer visibility sync protocol                            | In Progress | diagnostics can now send current Links/Fields layer visibility with SYNC LAYERS; the page-world runtime applies MapLibre layer visibility and uses visible layers for rendered-feature probes |
| Add page-world snapshot sync protocol                                    | In Progress | diagnostics and IRIS overlay can now send camera, source data, and layer visibility in one SYNC SNAPSHOT command; PAGE VISIBLE uses the same snapshot path for production-like initialization while keeping separate debug commands |
| Automatically mirror IRIS state into page-world runtime                  | In Progress | IRIS overlay sends an initial snapshot, then uses split updates: camera-only for map movement, data-only for entity changes, and layer-only for visibility changes |
| Test page-world runtime as full viewport map surface                     | In Progress | Diagnostics PAGE FULL hides the extension-world MapOverlay and promotes the MAIN-world runtime to a full viewport map above native INTEL UI but below IRIS controls; page-world camera changes feed back into IRIS map state |
| Add page-world selected feature and map-theme sync                       | In Progress | page-world runtime now receives selected portal/link/field feature collections for white highlights and updates raster base tiles when the IRIS map theme changes |
| Load live entities after moving in page-world full map                   | In Progress | page-world moveend now sends camera plus viewport bounds through the existing `IRIS_MOVE_MAP` request-coordinator path so far pans can trigger settled entity tile fetches |
...
| Migrate page-world source/layer updates                                  | Open   | replace diagnostic GeoJSON pushes with production-grade plain-JSON source updates for portals, links, fields, ornaments, artifacts, and plugin features |
| Migrate page-world interaction events                                    | Open   | page-world map should emit plain selection, hover, long-press, and draw-tool events back to extension UI without exposing MapLibre feature objects across worlds |
| Retire duplicate content-world MapLibre runtime after migration          | Open   | once page-world rendering owns the real map, remove temporary POC surfaces and keep only one production MapLibre runtime/bundle path |
