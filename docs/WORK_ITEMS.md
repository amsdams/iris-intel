...
| Split or memoize page-world GeoJSON/source sync by domain  | In Progress | selection sync now patches only selected sources, draw-tool state patches only planned features, and link/field layer visibility no longer empties source data; continue splitting entity/plugin/theme/filter rebuild paths |
