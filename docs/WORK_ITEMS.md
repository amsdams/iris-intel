...
| Add page-world snapshot sync protocol                                    | In Progress | diagnostics can now send camera, source data, and layer visibility in one SYNC SNAPSHOT command; PAGE VISIBLE uses the same snapshot path for production-like initialization while keeping separate debug commands |
