...
| Extract shared endpoint request gate helper                   | Done        | moved the pure in-flight/cooldown/freshness/queued request gate into `packages/core`; Mini-IRIS request queue uses it at enqueue and execution time, and IRIS entity refresh now uses it for in-flight and same-coverage freshness gating while endpoint-specific scheduling stays app-owned                                                                                                                                                                                                         |
