...
| Automatically mirror IRIS state into page-world runtime                  | In Progress | IRIS overlay now debounces store changes and sends a snapshot to the page-world runtime, so the bridge no longer depends only on manual Diagnostics buttons |
