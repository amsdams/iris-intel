...
| Record page-world benchmark improvement                        | Done   | page-world Bench samples show Chrome desktop at `17ms / 60fps / 0 slow`, Firefox desktop at `8ms / 122fps / 1 slow`, and Firefox mobile at `18ms / 57fps / 1 slow`; compare cautiously because viewport/source diagnostics are currently absent |
...
| Define fixed benchmark scenarios for version comparisons        | Open   | run the same scenario set for each migration: base map, default use, labels on, draw tools on, and worst reasonable case with tracker/labels/draw tools/keys; keep map area, zoom, style, browser, live/mock mode, and run count fixed |
...
| Restore page-world viewport/source benchmark diagnostics        | Open   | after retiring `MapOverlay`, Bench reports `VIEWPORT/SOURCES/HTML no sample`; add page-world source update timing and rendered feature counts so future comparisons show both frame and data costs |
| Rename or remove stale HTML marker diagnostics                  | Open   | page-world no longer uses the old HTML marker sync path, so Diagnostics should either hide HTML marker samples or replace them with page-world plugin/label render counters |
...
| Add page-world visual overlay parity                                     | In Progress | page-world full mode now receives and renders planned links/markers/crossings, artifacts, ornaments, mission route/waypoints, plugin GeoJSON line/point features, plugin label markers, and player marker labels |
...
| Align page-world map styling with existing IRIS MapOverlay               | In Progress | portal/link/field faction colours now come from the active IRIS theme, and page-world portal opacity, field opacity, link opacity, and portal stroke styling are closer to the existing renderer; remaining parity is detailed overlay polish |
| Finish page-world theme-settings parity                                  | In Progress | map tile theme sync uses MapLibre `setTiles`, and app theme changes now rebuild themed portal/link/field source colours; still verify repeated theme/map-style switching on Chrome and Firefox |
...
| Refine page-world player tracker styling and co-located players          | Open   | page-world player markers/labels differ slightly from the old HTML marker path; investigate how multiple players at the same portal should stack or aggregate |
| Add page-world long-press/contextmenu info open                          | In Progress | contextmenu/right-click opens info directly, but Firefox Mobile long-press did not trigger reliably; keep mobile long-press as a later interaction task |
...
| Verify repeated theme/map-style switching in page-world mode             | Open   | smoke-test Chrome, Firefox desktop, and Firefox Mobile for repeated app theme and base map style changes after the page-world runtime became default |
| Polish secondary overlay styling in page-world mode                      | Open   | compare artifacts, ornaments, mission routes/waypoints, plugin GeoJSON, plugin labels, planned items, and player tracker styling against old `MapOverlay`/IITC expectations |
| Restore page-world source/update diagnostics                             | Open   | Bench now reports frame timing from the MAIN-world runtime, but `VIEWPORT`, `SOURCES`, and `HTML` are `no sample`; add page-world source timing and feature counts before deeper perf tuning |
| Remove remaining page-world diagnostic naming/noise                      | Open   | PAGE POC controls are gone, but internal log/result names should be normalized around the production page-world runtime before merging |
| Decide whether page-world map runtime is worth a larger migration        | In Progress | POC succeeded enough to continue carefully on a feature branch; next slices should move camera, source, layer, and interaction ownership behind a strict plain-JSON command/event bridge |
...
| Retire duplicate content-world MapLibre runtime after migration          | In Progress | page-world rendering owns the normal map surface and the old `MapOverlay` fallback is no longer mounted; keep the old component file only until remaining styling/interaction parity is verified |
