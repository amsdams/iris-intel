...
| Suspend plugin marker registries during movement                 | In Progress | Marker-hidden and timing diagnostics showed z8 Normal still around `23-24ms avg`; movement overlay suspension now hides the same nonessential plugin/mission/planning layer set as `No Plugins`, plus planned/player marker registries |
