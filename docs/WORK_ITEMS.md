...
| Add diagnostics-only `queryRenderedFeatures` POC                         | In Progress | Diagnostics can now run direct, uncaught MapLibre `queryRenderedFeatures` probes for center/all, center/IRIS layers, viewport/all, and viewport/IRIS layers to verify Firefox extension behavior without production workarounds |
