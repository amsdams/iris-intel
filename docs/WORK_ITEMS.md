...
| Polish ornament ring visibility                                               | Done   | ornament rings now use a subtle yellow fill plus stronger zoom-scaled yellow strokes in both the page-world runtime and the old MapOverlay reference path                  |
...
| Finish page-world theme-settings parity                                  | In Progress | map tile theme sync uses MapLibre `setTiles`, app theme changes rebuild themed portal/link/field source colours, and the split sync paths were code-reviewed after source diagnostics landed; keep visual cross-browser smoke in the normal test pass |
...
| Polish secondary overlay styling in page-world mode                      | Open   | compare artifacts, mission routes/waypoints, plugin GeoJSON, plugin labels, and planned items against old `MapOverlay`/IITC expectations; ornaments have had a first-pass ring polish |
