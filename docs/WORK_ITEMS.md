...
| Test page-world runtime as full viewport map surface                     | Done   | page-world runtime can now run as the normal full-viewport IRIS map surface above native INTEL UI but below IRIS controls; the extension-world MapOverlay remains as fallback during migration |
...
| Make page-world runtime the default IRIS map surface                     | In Progress | normal IRIS startup now promotes the MAIN-world runtime to full map mode by default while keeping the old extension-world MapOverlay path available as a fallback |
...
| Verify player tracker rendering in page-world mode                       | Open   | page-world now has explicit player marker and label layers, but live COMM/player visibility still needs Chrome/Firefox verification with active tracker data |
| Add page-world long-press/contextmenu info open                          | In Progress | contextmenu/right-click opens info directly; verify whether mobile browser long-press reliably emits the same MapLibre contextmenu event |
