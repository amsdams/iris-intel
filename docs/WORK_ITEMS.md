...
| Align player tracker freshness policy                      | Open   | IRIS currently behaves more history-oriented while Mini-IRIS feels more recent-feed oriented; decide later whether both default to recent-only, retained 3h history, or clearer labels/options |
...
| Review Machina map colours across IRIS and Mini-IRIS       | Open   | expected default is red-aligned Machina styling; Mini-IRIS currently appears purple for pins and IRIS appears grey in at least one smoke test, so compare team colour constants, portal pins, links, fields, and player/activity markers |
| Review neutral portal map colours across IRIS and Mini-IRIS | Open   | expected default is a white border/ring with a grey portal body; verify base portal styling, level/health modes, selected state, and Mini-IRIS vs IRIS parity |
