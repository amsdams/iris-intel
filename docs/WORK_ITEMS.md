...
| Align page-world map styling with existing IRIS MapOverlay               | Open   | page-world layers still use simpler hardcoded paint values for some overlays; compare faction colours, highlight widths, field opacity, and planned-item styling before retiring the old renderer |
| Finish page-world theme-settings parity                                  | Open   | map tile theme sync exists, but verify repeated theme changes and decide whether plugin/app theme colours should also affect page-world layer paint values |
