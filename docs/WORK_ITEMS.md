...
| Tune low-zoom moving-mode link rendering                       | Done   | Page-world runtime hides links during active movement at z8.5 and below and diagnostics confirmed z8 Normal runs with `links none`; remaining z8 Normal vs No Plugins gap is now tracked as moving marker/overlay suspension |
...
| Compare stationary vs moving field-render modes                 | Done   | Page-world runtime hides fields during active movement at z10.5 and below and diagnostics confirmed z8 Normal runs with `fields none`; remaining z8 Normal vs No Plugins gap is now tracked as moving marker/overlay suspension |
...
| Resume label-heavy overlays after a short settle delay          | Done   | Page-world plugin, mission, planning, label, and highlight layers now hide during active movement and restore after a short settle delay |
| Suspend plugin marker registries during movement                 | Done   | Movement overlay suspension now hides the same nonessential plugin/mission/planning layer set as `No Plugins`, plus planned/player marker registries; final z8 Normal batch improved only slightly to `22ms avg`, so the remaining gap needs Chrome Performance trace-level profiling |
| Profile residual z8 Normal vs No Plugins gap                     | Open   | With links, fields, plugin layers, and marker registries hidden, z8 Normal still trails `No Plugins` by about `5ms`; do not block Mini-IRIS on this without a Chrome Performance trace |
