...
| Add portal popup action row                      | Open   | first CSS-only action row wrapper now holds Missions Starting Here; continue by adding high-frequency actions such as copy/navigation or map movement near the top without removing details |
| Extract popup action row primitive               | Open   | first portal action-row class exists; factor repeated action-button layout into a shared popup primitive once portal/link/field/player/plugin details have more than one common action |
