...
| Make entity refresh generation-aware after major view changes          | Open   | queued stale-view tile batches should be dropped or deprioritized when zoom/bounds change substantially; in-flight HTTP can finish but stale generation results should not delay newer current-view refresh work |
