...
| Address can disappear or break in diagnostics after smoke test          | Done   | diagnostics now actively reverse-geocodes the current map center when no fresh center address is cached, location updates preserve the coordinate they describe, reverse-geocode status returns to idle on empty/non-OK responses, and address-cache transitions are covered by shared core tests |
...
| Task                                                                     | Status | Notes                                                                                                                                                                                                                                                                                                                             |
|--------------------------------------------------------------------------|--------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Split map sync store actions by intent                                   | Done   | shared core now exposes pure map camera/viewport transition helpers plus explicit store methods; IRIS runtime callers and Mini-IRIS local page-map state both use the shared camera-only vs bounded-viewport semantics                                                                                                           |
...
| Isolate reverse-geocode state into a dedicated module or slice           | Done   | pure address normalization, cache-skip, and resolved-address merge logic now lives in `packages/core/src/address-state.ts`; the store still owns debounce/network side effects for now                                                                                                                                              |
| Tighten IRIS message-type contracts for map sync                         | Done   | IRIS-owned map moves, region-score requests, and portal-detail requests now pass through typed runtime parsers before the coordinator mutates map state or posts backend fetch messages                                                                                                                                             |
...
1. Runtime convergence audit: compare IRIS core-store ownership with Mini-IRIS local page-map state and only merge
   ownership when the semantics are truly the same; otherwise share pure reducers/helpers first.
2. Live update pipeline: reduce perceived COMM-to-map lag with targeted, throttled refreshes from actionable plexts
   before considering broader polling.
3. Page-world map protocol helpers: share typed protocol helpers only after IRIS and Mini-IRIS call sites stabilize.
4. Map style/domain constants: continue consolidating canonical Intel/Ingress constants and derive non-default theme
   variants without making app UI themes shared.
5. Diagnostics/bench formatting: keep copied bench/readout fields aligned through shared formatting helpers where
...
| Extract shared entity metric display helpers                  | Done        | moved rough field-MU estimation and precise/compact distance formatting into `packages/core`; IRIS and Mini-IRIS field/link details and drawer distance labels now share the same calculation while keeping layout local                                                                                                                                                                                                                                                                            |
...
| Extract shared COMM window replacement rule                   | Done        | moved the full-current-window `getPlexts` replacement decision into `packages/core`; IRIS and Mini-IRIS now share when COMM responses replace visible messages versus merge incrementally                                                                                                                                                                                                                                                                                                         |
| Improve IRIS move-triggered local COMM freshness              | Done        | added a shared meaningful-bounds-change helper and made IRIS bypass its short COMM cooldown only when the viewport moved/resized enough; regular polling and small-pan cooldown behavior stay unchanged                                                                                                                                                                                                                                                                                            |
| Extract shared extension request message builders             | Done        | moved stable portal-details, entity, inventory, artifact, subscription, player-stats, game-score, and region-score message constructors into `packages/core`; apps still own when to request, cooldowns, endpoint scheduling, and UI actions                                                                                                                                                                                                                                                     |
...
| Extract canonical team normalization helper                   | Done        | moved Intel/Ingress team normalization into a dedicated core module and switched parsers, plugin API, and player tracker reduction away from local/barrel definitions; Machina aliases now stay consistent across app and runtime paths                                                                                                                                                                                                                                                              |
| Extract shared player level/AP display derivation             | Done        | moved stable AP formatting and next-level progress calculation into `packages/core`; IRIS and Mini-IRIS player panels now share the same clamped progress semantics while keeping popup layout and colors local                                                                                                                                                                                                                                                                                    |
| Extract shared portal history semantics                       | Done        | moved visited/captured/scanned history keys, overlay modes, default-off layer state, mode cycling, persisted-mode validation, overlay flag derivation, and All/Yes/No filter matching into `packages/core`; apps keep ring rendering and filter UI local                                                                                                                                                                                                                                             |
| Add Mini-IRIS-style portal history rings to IRIS              | Done        | IRIS page-world rendering now draws visited/captured/scanned highlight and inverse rings from the same portal source using canonical history colours and Mini-IRIS radius offsets; history rings default off and have explicit Tactical drawer controls, while existing All/Yes/No history filters remain tactical hide/show controls                                                                                                                                           |
| Align IRIS and Mini-IRIS diagnostics copy/read order          | Done        | Mini-IRIS DBG now keeps human-readable fields first and aligns copied bench field names/order with IRIS around environment, z/mode, items, visible counts, sources, frame stats, render/query, and toggles; shared core helpers now format neutral diagnostic counts/timings, endpoint telemetry parsing, browser/environment labels, and shared benchmark scenario order while app-specific bench line assembly stays app-owned                                                                  |
...
| Refine viewport-scoped COMM cache and small-pan merge policy  | In Progress | shared current-window replacement and meaningful-bounds-change rules now keep far-away COMM from lingering and let IRIS refresh local COMM faster after larger pans; later consider bounds/region buckets or small-pan append/merge so returning to a recent area can reuse local COMM history without showing unrelated regions                                                                                                                                                                  |
| Add shared mock Intel/domain object builders                  | Done        | added small deterministic builders for parser-contract fixtures and domain objects in `packages/core`, starting with entity map payloads, portal details, portals, links, and plexts; Mini-IRIS pattern-mode topology and IRIS-specific debug UI flows stay app-owned                                                                                                                                                                                                                              |
| Audit remaining non-UI shared runtime candidates              | Open        | keep as an audit list, not an extraction mandate: live topology freshness tuning, COMM region-cache policy, mock data generators, remaining app-specific extension protocol builders, page-map protocol helpers, and any request/data helpers that gain a second concrete call site                                                                                                                                                                                                                 |
| Refine COMM-driven topology refresh policy                    | Done        | added shared current-view COMM hint filtering and topology refresh gate decisions; IRIS uses the shared decision for its delayed entity refresh, and Mini-IRIS now performs a conservative current-view `getEntities` refresh after actionable COMM only when the tile count is small enough                                                                                                                                                                                                          |
...
| Extract shared plext debug snapshot formatter                 | Done        | moved pure raw/parsed COMM debug snapshot formatting into `packages/core`; Mini-IRIS uses it for DBG copy actions while apps still own diagnostics buttons and any sensitive-data copy affordances                                                                                                                                                                                                                                                                                                |
...
| Extract shared portal radius derivation                       | Done        | moved Intel-style zoom-to-portal-radius interpolation into `packages/core` so Mini-IRIS generated portal feature props and page-world MapLibre radius expressions share the same canonical style stops                                                                                                                                                                                                                                                                                              |
| Extract shared resonator energy derivation                    | Done        | moved Intel resonator max-energy, resonator energy percent, and portal resonator energy summary helpers into `packages/core`; IRIS and Mini-IRIS portal detail views now share the same energy math while keeping layout and colours local                                                                                                                                                                                                                                                          |
...
| Extract shared entity request batching helper                 | Done        | moved the canonical 25-tile Intel entity request batch size and tile-key chunking helper into `packages/core`; IRIS entity refresh, IRIS benchmark preload diagnostics, and Mini-IRIS live/preload/COMM entity refresh now use the same batching rule while keeping scheduling and endpoint telemetry app-owned                                                                                                                                                                                       |
...
| Use shared camera validation at runtime sync boundaries       | Done        | IRIS incoming Intel startup/position messages and Mini-IRIS Intel-map sync messages now pass through the shared camera parser before clamping or applying map movement, keeping non-finite/out-of-range camera payloads from crossing runtime boundaries                                                                                                                                                                                                                                          |
...
| Extract shared portal-details merge preparation               | Done        | moved parse-plus-merge preparation for portal-details responses into `packages/core`; IRIS and Mini-IRIS now get the same merged portal result while keeping request timing, store update side effects, selected-state handling, and logging app-owned                                                                                                                                                                                                                                               |
| Extract shared plext debug and entity display predicates      | Done        | moved pure COMM raw/parsed debug snapshot formatting, portal/link/field visibility predicates, and artifact/ornament display classification into `packages/core`; apps still own diagnostics buttons, layer toggles, feature styling, and MapLibre layer definitions                                                                                                                                                                                                                                   |
...
| Extract shared persisted settings schema helpers              | Done        | moved reusable persisted map-camera parsing, nested Zustand-state record reads, string-choice parsing, and portal-history layer-state normalization into `packages/core`; IRIS and Mini-IRIS now share validation for startup map camera and history ring defaults while keeping storage keys/app defaults local                                                                                                                                                                                        |
...
| Extract shared endpoint request gate helper                   | Done        | moved the pure in-flight/cooldown/freshness/queued request gate into `packages/core`; Mini-IRIS request queue uses it at enqueue and execution time, and IRIS entity, game-score, and region-score refreshes now use it for matching in-flight/freshness gates while endpoint-specific scheduling stays app-owned                                                                                                                                                                                   |
| Extract shared endpoint telemetry polling gate                | Done        | moved Mini-facing telemetry skip and next-delay helpers into `packages/core`; Mini-IRIS COMM, score, subscription, inventory, and player-tracker polling hooks now share the same in-flight/cooldown/fresh telemetry checks while IRIS keeps store-diagnostic request policy local                                                                                                                                    |
| Extract shared runtime message and protocol primitives        | Done        | moved generic page/runtime message guards, Mini endpoint telemetry parsing, shared benchmark scenario constants, and common runtime camera/bounds aliases into `packages/core`; app-specific page-map command unions, styling payloads, and runtime orchestration stay local                                                                                                                                                                                                                        |
| Share pure map camera/viewport transition semantics           | Done        | `packages/core` now owns pure camera-only and bounded-viewport update helpers; IRIS core store methods and Mini-IRIS local page-map state both use them, while full store ownership remains intentionally separate                                                                                                                                                                                                                                                                                 |
| Decide Mini-IRIS vs IRIS runtime convergence model            | Open        | clarify where Mini should keep compact local state versus adopting IRIS/core store ownership; current preference is incremental sharing of pure semantics first, then revisit merging only if duplicate orchestration or behavior drift becomes costly                                                                                                                                                                                                                                             |
| Audit remaining page-map protocol overlap                     | In Progress | current audit says do not merge the full protocols yet: IRIS uses named messages with E6 bounds and richer layer/plugin/planning payloads, while Mini uses a compact command/event envelope with degree bounds; shared only stable primitives so far, including camera/bounds/view, benchmark modes, entity selection kind/intent vocabulary, and IRIS legacy `openInfo` to shared intent conversion                                      |
| Decide COMM region-cache/small-pan policy                     | Open        | current rules avoid stale far-away COMM but do not yet cache recent local COMM by viewport/region; investigate only if user-visible COMM history loss or repeated fetch noise becomes a practical problem                                                                                                                                                                                                                                                                                          |
| Decide debug mock fixture release boundary                    | Open        | mock builders are shared, but app debug fixtures and JSON payloads can still ship in release bundles; decide whether to gate them behind dev-only entry points, build flags, or sanitized generated fixtures                                                                                                                                                                                                                                                                                       |
...
| Add entity request tile/batch deltas             | Done   | batch and preload rows now include current tile count, fresh tile count, batch count, data zoom, active/passive entity successes, stale generation drop/ignore deltas, and the scheduler pass `req/total/fresh/batches` summary |
| Add compact entity pass lifecycle summary        | Done   | entity diagnostics now track the latest pass id, generation, reason, requested/total/fresh tile counts, batch count, and data zoom; copied benchmark rows label it as `current` or `carry` so leftover async work is visible |
| Wait for a quiet window between batch scenarios  | Done   | Batch now waits for no active requests, no in-flight entity endpoint, and a short endpoint-activity quiet period before preloading/scenario timing, reducing carryover from the previous row |
...
| Add compact source publication pass summary         | Done   | page-world source publications now carry a `sourcePass` id/reason/current-or-carry summary in copied preload and timed rows, including window pass count, moving pass count, latest-pass moving state, sources touched, real `setData` calls, unchanged skips, and pass `setData` time |
...
| Add per-scenario UI render deltas                | Done | copied rows now include `ui renders` with the top changed UI surfaces from existing render diagnostics; first desktop validation showed mostly `none` or low-count toolbar/dock/status renders and no UI-churn bottleneck |
...
| Add manual interaction capture mode              | Done | `Bench Options` now exposes `Capture Drag`, which records an 8s real interaction window and reports the same net/source/longtask/UI/workload fields; first desktop z16 capture was clean at `avg 9ms`, `fps 117`, `slow 1/937`, `setData 2ms`, and no long tasks |
...
| Review 'soft', 'cyber' and 'debug' colors. can we use RGB to HSL and formulas | Done | IRIS derives broad CYBER and SOFTER palettes from INGRESS through module-load HSL transforms, with explicit overrides for key/powerup/Aegis/neutral values; history, health, mission, artifact, ornament, and dock accent now read from the active theme, and DEBUG has automated dark-surface contrast checks |
