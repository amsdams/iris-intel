...
2. **Interaction-aware render/source queue**
   ...
   that suspends the heavy main link/field layers during active movement. The next practical gap is to make source
   publication pass ownership clearer: attach a reason/pass id to source publication, suppress unchanged sources more
   aggressively, and keep selection/plugin/planning updates distinguishable from entity refresh updates.
